...
void interpHeader(std_msgs::msg::Header header_1,
  ...
  // Convert stamps to rclcpp::Time
  rclcpp::Time time1(header_1.stamp);
  rclcpp::Time time2(header_2.stamp);

  rclcpp::Duration state_duration = time2 - time1;
  int64_t interp_ns =
...
void interpOdometry(quad_msgs::msg::BodyState state_1,
  ...
  interp_state.pose.position.x = math_utils::lerp(
      state_1.pose.position.x, state_2.pose.position.x, t_interp);
  interp_state.pose.position.y = math_utils::lerp(
      state_1.pose.position.y, state_2.pose.position.y, t_interp);
  interp_state.pose.position.z = math_utils::lerp(
      state_1.pose.position.z, state_2.pose.position.z, t_interp);

  // Interp body orientation with math_utils::lerp
...
void interpJointState(sensor_msgs::msg::JointState state_1,
  ...
  for (int i = 0; i < state_1.position.size(); i++) {
    ...
    interp_state.position[i] =
        math_utils::lerp(state_1.position[i], state_2.position[i], t_interp);
    interp_state.velocity[i] =
        math_utils::lerp(state_1.velocity[i], state_2.velocity[i], t_interp);
    interp_state.effort[i] =
        math_utils::lerp(state_1.effort[i], state_2.effort[i], t_interp);
  }
}
...
void interpGRFArray(quad_msgs::msg::GRFArray state_1,
  ...
  for (int i = 0; i < interp_state.vectors.size(); i++) {
    interp_state.vectors[i].x =
        math_utils::lerp(state_1.vectors[i].x, state_2.vectors[i].x, t_interp);
    interp_state.vectors[i].y =
        math_utils::lerp(state_1.vectors[i].y, state_2.vectors[i].y, t_interp);
    interp_state.vectors[i].z =
        math_utils::lerp(state_1.vectors[i].z, state_2.vectors[i].z, t_interp);

    interp_state.points[i].x =
        math_utils::lerp(state_1.points[i].x, state_2.points[i].x, t_interp);
    interp_state.points[i].y =
        math_utils::lerp(state_1.points[i].y, state_2.points[i].y, t_interp);
    interp_state.points[i].z =
        math_utils::lerp(state_1.points[i].z, state_2.points[i].z, t_interp);
...
void interpRobotState(quad_msgs::msg::RobotState state_1,
  ...
  interpJointState(state_1.joints, state_2.joints, t_interp,
                   interp_state.joints);
...
void interpRobotPlan(quad_msgs::msg::RobotPlan msg, double t,
  ...
  rclcpp::Time state_1_stamp(state_1.header.stamp);
  rclcpp::Time state_2_stamp(state_2.header.stamp);
  rclcpp::Duration t1_ros = state_1_stamp - t0_ros;
...
void ikRobotState(const quad_utils::QuadKD &kinematics,
  ...
  for (int i = 0; i < multi_foot_state.feet.size(); i++) {
    ...
    kinematics.worldToFootIKWorldFrame(i, body_pos, body_rpy, foot_pos,
                                       leg_joint_state);
  ...
  Eigen::VectorXd ref_body_state(12), ref_foot_positions(12),
      ref_foot_velocities(12);

  // Load state data
  ref_body_state = quad_utils::bodyStateMsgToEigen(body_state);
  quad_utils::multiFootStateMsgToEigen(multi_foot_state, ref_foot_positions,
                                       ref_foot_velocities);
  ...
  quad_utils::vectorToEigen(joint_state.position, joint_positions);
...
quad_msgs::msg::BodyState eigenToBodyStateMsg(const Eigen::VectorXd &state) {
  quad_msgs::msg::BodyState state_msg;
...
void multiFootStateMsgToEigen(
  ...
  multiFootStateMsgToEigen(multi_foot_state_msg, foot_positions,
                           foot_velocities);
