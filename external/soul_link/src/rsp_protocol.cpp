...
void RspProtocol::RunMotor(int motorId, float torque, float angle,
  ...
  data[2] =
      float_to_uint(angular_speed, m->m_velRange[0], m->m_velRange[1], 16) >> 8;
  data[3] =
      float_to_uint(angular_speed, m->m_velRange[0], m->m_velRange[1], 16) &
      0xff;
  data[4] = float_to_uint(kp, m->m_kpRange[0], m->m_kpRange[1], 16) >> 8;
  data[5] = float_to_uint(kp, m->m_kpRange[0], m->m_kpRange[1], 16) & 0xff;
  data[6] = float_to_uint(kd, m->m_kdRange[0], m->m_kdRange[1], 16) >> 8;
  data[7] = float_to_uint(kd, m->m_kdRange[0], m->m_kdRange[1], 16) & 0xff;
  uint32_t itorque = float_to_uint(torque, m->m_torqueRange[0],
                                   m->m_torqueRange[1], 16); // 注意电机型号
...
void RspProtocol::handleCanMsg(const iTek_CANFD_Receive_Data &recvdata) {
  ...
  case 0x00: {
    ...
    if ((frameid & 0xff) == 0xfe) {
      ...
      printf(
          "recv motor %d chip id: [%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X]\n",
          motorId, recvdata.frame.data[0], recvdata.frame.data[1],
