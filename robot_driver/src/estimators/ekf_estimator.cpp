...
void EKFEstimator::init() {
  ...
  quad_utils::loadROSParam(node_, "topics.state.joints", joint_encoder_topic);
  ...
  quad_utils::loadROSParam(node_, "topics.contact_mode", contact_topic);
  ...
  quad_utils::loadROSParam(node_, "robot_driver.joint_state_max_time",
  ...
  quad_utils::loadROSParam(node_, "robot_driver.is_hardware", is_hardware_);

  // Setup subs
  joint_encoder_sub_ = node_->create_subscription<sensor_msgs::msg::JointState>(
      joint_encoder_topic, 10,
      std::bind(&EKFEstimator::jointEncoderCallback, this,
                std::placeholders::_1));
  imu_sub_ = node_->create_subscription<sensor_msgs::msg::Imu>(
      imu_topic, 10,
      std::bind(&EKFEstimator::imuCallback, this, std::placeholders::_1));
  grf_sub_ = node_->create_subscription<quad_msgs::msg::GRFArray>(
      grf_topic, 10,
      std::bind(&EKFEstimator::grfCallback, this, std::placeholders::_1));
  contact_sub_ = node_->create_subscription<quad_msgs::msg::ContactMode>(
      contact_topic, 10,
      std::bind(&EKFEstimator::contactCallback, this, std::placeholders::_1));
  state_ground_truth_sub_ =
      node_->create_subscription<quad_msgs::msg::RobotState>(
          ground_truth_topic, 10,
          std::bind(&EKFEstimator::groundtruthCallback, this,
                    std::placeholders::_1));

  // QuadKD class
  quadKD_ = std::make_shared<quad_utils::QuadKD>(node_);

  RCLCPP_INFO(node_->get_logger(), "EKF_Estimator Initiated");
}

bool EKFEstimator::updateOnce(quad_msgs::msg::RobotState &estimated_state_) {
  ...
  if (last_grf_msg_ != nullptr) {
    if (initialized) {
      ...
      // Initialize Start Pose of the Robot to Standing Pose
...
void EKFEstimator::imuCallback(const sensor_msgs::msg::Imu::SharedPtr msg) {
  last_imu_msg_ = msg;
}

void EKFEstimator::contactCallback(
    const quad_msgs::msg::ContactMode::SharedPtr msg) {
  last_contact_msg_ = msg;
}

void EKFEstimator::grfCallback(const quad_msgs::msg::GRFArray::SharedPtr msg) {
  last_grf_msg_ = msg;
}
...
quad_msgs::msg::RobotState EKFEstimator::StepOnce() {
  ...
  // calculate dt
  double dt = (start_time - last_time).seconds();
  ...
  /// publish new message
  new_state_est.header.stamp = node_->now();
  ...
  // joint
  new_state_est.joints.header.stamp = node_->now();
...
void EKFEstimator::update(const Eigen::VectorXd &jk, const Eigen::VectorXd &fk,
  ...
  //       (*last_robot_state_msg_).feet.feet[3].position.z, X_pre.segment(22,
  //       3), X_pre.segment(25, 3);
  // }
  ...
  // Skip Update if the Norm of the Innovation is too high
  ...
  // Reject Encoder Update and Proceed Only with Prediction
  else {
    X = X_pre;
    P = P_pre;
  }
