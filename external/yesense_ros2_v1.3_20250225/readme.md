...
或者使用set_lib_path.sh脚本一键设置

4. 生成msg消息头文件
colcon build --packages-select yesense_interface（在更改msg消息格式时需重新编译）或着把该命令添加到build.sh脚本中

或者使用gen_interface.sh脚本来进行生产
...
- 在默认该驱动使用的serial库是基于X86平台的，如果使用ARM平台（如jeston nano）的话需要使用源码自行编译看，然后将编译后的serial库
放在src/yesense_std_ros2/src/serial下
