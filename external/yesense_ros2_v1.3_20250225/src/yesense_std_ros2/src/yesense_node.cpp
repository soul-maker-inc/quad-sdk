...
int g_baudrate_table[BAUDRATE_CNT] = {
    ...
    115200, 230400, 460800, 921600}; // 目前仅支持这些波特率

int g_baudrate_table_linux[BAUDRATE_CNT] = {
    B9600,   B19200,  B38400,  B57600,
    B115200, B230400, B460800, B921600}; // 目前仅支持这些波特率
...
public:
  YESENSE_Publisher() : Node("yesense_publisher") {
    pub_imu_ros =
        this->create_publisher<sensor_msgs::msg::Imu>("imu_data_ros", 10);
    pub_pose_geometry =
        this->create_publisher<geometry_msgs::msg::PoseStamped>("pose_geo", 10);
    pub_marker =
        this->create_publisher<visualization_msgs::msg::Marker>("marker", 10);

    // ==========================================
    pub_imu =
        this->create_publisher<yesense_interface::msg::ImuData>("imu_data", 10);
    pub_sensor_10axis =
        this->create_publisher<yesense_interface::msg::ImuDataTenAxis>(
            "sensor_10axis", 10);
    pub_euler = this->create_publisher<yesense_interface::msg::EulerOnly>(
        "euler_only", 10);
    pub_robt_lord = this->create_publisher<yesense_interface::msg::RobotLord>(
        "robot_lord", 10);
    pub_att_min_vru =
        this->create_publisher<yesense_interface::msg::AttitudeMinVru>(
            "att_min_vru", 10);
    pub_att_min_ahrs =
        this->create_publisher<yesense_interface::msg::AttitudeMinAhrs>(
            "att_min_ahrs", 10);
    pub_att_all =
        this->create_publisher<yesense_interface::msg::AttitudeAllData>(
            "att_all", 10);
    pub_pos =
        this->create_publisher<yesense_interface::msg::PosOnly>("pos_only", 10);
    pub_nav_min =
        this->create_publisher<yesense_interface::msg::NavMin>("nav_min", 10);
    pub_nav_min_utc = this->create_publisher<yesense_interface::msg::NavMinUtc>(
        "nav_min_utc", 10);
    pub_nav_all =
        this->create_publisher<yesense_interface::msg::NavAll>("nav_all", 10);
    ...
    *((unsigned int *)&yis_out) = 0u;
    ...
    if (0 ==
        memcmp(driver_type_str.c_str(), "ros_serial", strlen("ros_serial"))) {
      driver_type = serial_drv_ros;
    } else if (0 == memcmp(driver_type_str.c_str(), "linux_serial",
                           strlen("linux_serial"))) {
    ...
    timer_ = this->create_wall_timer(
        2ms, std::bind(&YESENSE_Publisher::timer_callback, this));
    // timer_msg_rate_ = this->create_wall_timer(1ms,
    // std::bind(&YESENSE_Publisher::callback_msg_rate_calc, this));
  }
...
private:
  void timer_callback() {
    ...
    int ret = decoder.data_proc(r_buffer, (unsigned int)bytes_read_r_buffer,
                                &yis_out);
    if (analysis_ok == ret) {
      if (yis_out.content.valid_flg) {
        yis_out.content.valid_flg = 0u;
        user_info.msg_cnt++;
        // RCLCPP_INFO(this->get_logger(), "msg rate: %d, tid %d, acc: %f, %f,
        // %f gyro: %f, %f, %f, euler: %f, %f, %f!", 			user_info.msg_rate,
        //			yis_out.tid, yis_out.acc.x, yis_out.acc.y,
        //yis_out.acc.z, yis_out.gyro.x, yis_out.gyro.y, yis_out.gyro.z,
        //			yis_out.euler.pitch, yis_out.euler.roll,
        //yis_out.euler.yaw
        //		);
        publish_msg(&yis_out);
      }
  ...
  rclcpp::Publisher<yesense_interface::msg::ImuData>::SharedPtr
      pub_imu; // only inertia data: tid/acc/gyro/sensor_temp/sample_timestamp
  rclcpp::Publisher<yesense_interface::msg::ImuDataTenAxis>::SharedPtr
      pub_sensor_10axis; // only inertia data:
                         // tid/acc/gyro/sensor_temp/sample_timestamp/mag/pressure(reserved)
  rclcpp::Publisher<yesense_interface::msg::EulerOnly>::SharedPtr
      pub_euler; // only euler: tid/pitch/roll/yaw
  rclcpp::Publisher<yesense_interface::msg::RobotLord>::SharedPtr
      pub_robt_lord; // tid/acc/gyro/quatenion
  rclcpp::Publisher<yesense_interface::msg::AttitudeMinVru>::SharedPtr
      pub_att_min_vru; // minimum recommended data for attitude vru product:
                       // imu/euler
  rclcpp::Publisher<yesense_interface::msg::AttitudeMinAhrs>::SharedPtr
      pub_att_min_ahrs; // minimum recommended data for attitude ahrs product:
                        // imu/mag/euler
  rclcpp::Publisher<yesense_interface::msg::AttitudeAllData>::SharedPtr
      pub_att_all; // attitude all data: imu/mag/euler/quat
  rclcpp::Publisher<yesense_interface::msg::PosOnly>::SharedPtr
      pub_pos; // only position: tid/latitude/longitude/altitude/nav_state
  rclcpp::Publisher<yesense_interface::msg::NavMin>::SharedPtr
      pub_nav_min; // recommended data for navigation product:
                   // tid/postion/euler/nav_state
  rclcpp::Publisher<yesense_interface::msg::NavMinUtc>::SharedPtr
      pub_nav_min_utc; // recommended data for navigation product with utc:
                       // tid/postion/euler/utc/nav_state
  rclcpp::Publisher<yesense_interface::msg::NavAll>::SharedPtr
      pub_nav_all; // navigation all data:
                   // tid/acc/gyro/sensor_temp/eluer/quat/position/nav_state/vel/utc/pressure(reserved)
...
void YESENSE_Publisher::publish_msg(yis_out_data_t *result) {
  ...
  sensor_data_10axis.imu_basic.sample_timestamp.timestamp =
      result->sample_timestamp;
  ...
  att_min_vru_data.imu_basic.sample_timestamp.timestamp =
      result->sample_timestamp;
  ...
  att_min_ahrs_data.att_min_vru.imu_basic.sample_timestamp.timestamp =
      result->sample_timestamp;
  ...
  att_all_data.att_min_ahrs.att_min_vru.imu_basic.temp.temp =
      result->sensor_temp;
  att_all_data.att_min_ahrs.att_min_vru.imu_basic.sample_timestamp.timestamp =
      result->sample_timestamp;
  ...
  nav_min_utc_data.nav_basic.pos.status.fusion_status =
      result->status.status.bit.fusion_sta;
  nav_min_utc_data.nav_basic.pos.status.gnss_status =
      result->status.status.bit.pos_sta;
